...
public:
  DecisionNode() : Node{"decision"} {
    ...
    client = create_client<ros2_kdb_msgs::srv::QueryDatabase>(
        "/kdb_server/sparql_query");
    subscriber =
        create_subscription<nav_msgs::msg::Odometry>("odom", 10, std::bind(&DecisionNode::pose_callback, this, std::placeholders::_1));
  }
...
private:
  ...
  Client<ros2_kdb_msgs::srv::QueryDatabase>::SharedPtr client;
  geometry_msgs::msg::Point current_position_;

  void decision_callback(const Goals::SharedPtr msg) {
    RCLCPP_INFO(get_logger(), "Got goal request for destination: %s",
                msg->goals[0].destination.c_str());
    ...
    auto response = client->async_send_request(
        request,
        [this, goals](
            Client<ros2_kdb_msgs::srv::QueryDatabase>::SharedFuture future) {
          RCLCPP_INFO(get_logger(), "Hello guys");
          ...
          if (goal.type == "goto") {
            ...
            if (vending != vec.end() && guy != vec.end()) {
              ...
              children.append(make_drive_to_node(*vending));
              children.append(make_drive_to_node({"","","",current_position_.x, current_position_.y}));
            }
          } else {
            QJsonObject jsonObj{
                {"children", QJsonArray()},
  ...
  void pose_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    // Update the member variable with the received position information
    current_position_ = msg->pose.pose.position;
    RCLCPP_INFO(get_logger(), "Received position: %f %f %f",
